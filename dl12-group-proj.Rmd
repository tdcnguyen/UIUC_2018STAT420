...
md_mass_inter = step(md_start, 
                     scope = log(pl_bmasse) ~ pl_discmethod + pl_disc + pl_locale + log(st_dist) + st_optmag + log(st_mass) + log(st_rad) + exp(st_metfe) + st_vsini + pl_orbeccen + log(pl_orbper) + pl_locale:pl_orbeccen + log(st_mass):st_vsini +  pl_locale:log(st_dist) + log(st_dist):st_vsini + log(st_dist):log(pl_orbper) + st_vsini:log(pl_orbper) + pl_orbeccen:log(pl_orbper) + log(st_rad): st_logg, 
                     direction = "both", 
                     trace = 0)
